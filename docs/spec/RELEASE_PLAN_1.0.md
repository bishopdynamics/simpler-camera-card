...
- 2026-08-28 — **Round 2 code work complete, v0.7.0** (`e5ef305`). Batches: 1 card.ts fixes (`de00457`), 2 cross-cutting (`c9d779b`), 3 simplification review (20 findings, verdict "better than most 1.0 open-source TypeScript"), 4 source cleanup (`708711a` — API surface trimmed to 7 exports, doc-drift purged, shared logging/errors leaf, same-origin helper dedup, dead exports removed; pushed back on watchdog.destroy-in-stop: stop→start reuse is a tested path), 5 test consolidation (`0633137` — tests/fixtures.ts, tests/readme.test.ts de-brittled, net −375 lines). Personal dev state untracked (`9b8f631`: dev.sh, .bishop/). 347 unit + 3 integration tests; prettier/eslint/tsc clean. **Awaiting James: round-2 sign-off + two decisions:** (a) GUI can save transient-invalid configs — accept or guard; (b) optional status-pill "attempt N" enhancement (from SupervisorStateDetail finding).
