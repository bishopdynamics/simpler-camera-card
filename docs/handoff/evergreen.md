...
- **Releases:** `dist/` is committed (HACS/manual installs); every user-visible change bumps the version in `package.json` + `src/index.ts` (`CARD_VERSION`) and rebuilds dist, because dashboards cache-bust via `?v=`.
- **Repos & push policy (2026-08-28):** `origin` = dev repo (git.bishopdynamics.com) — the only remote the agent ever pushes to. `github` = public repo `https://github.com/bishopdynamics/simpler-camera-card` — **only James pushes there; pushing it is the release act.** Public docs reference the GitHub URL. No CI, no HACS default-store submission (users add the repo as a HACS custom repository). Feature-complete declared at v0.6.1 (2026-08-28); v1.0.0 happens only on James's explicit final word (see `docs/spec/RELEASE_PLAN_1.0.md`).
